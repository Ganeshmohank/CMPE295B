...
def demo_related_links_for_title(title: str) -> list[dict[str, str]]:
    """Dummy external URLs (Atlassian-style) for Related content; replace with real integrations later."""
    root = "https://example.atlassian.net"
    if title == "Security Review: Auth Hardening":
        ...
        return links
    return [
        {"title": f"Initiative — {title[:40]}", "url": f"{root}/jira/software/projects/DEMO"},
...
async def seed_database() -> None:
    ...
    for spec in specs:
        ...
        meeting_doc["related_links"] = demo_related_links_for_title(spec["title"])
