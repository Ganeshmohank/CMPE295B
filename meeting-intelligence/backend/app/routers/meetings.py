...
async def patch_meeting_context(
    ...
    await get_db().meetings.update_one({"_id": oid}, {"$set": updates})
...
async def meeting_detail(meeting_id: str) -> MeetingDetailResponse:
    ...
    raw_links = m.get("related_links") or []
    related_out: list[RelatedLinkOut] = []
    for x in raw_links:
        if isinstance(x, dict) and x.get("title") and x.get("url"):
            related_out.append(RelatedLinkOut(title=str(x["title"]).strip(), url=str(x["url"]).strip()))
