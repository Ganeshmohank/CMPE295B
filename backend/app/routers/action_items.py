...
async def review_queue(
    ...
    meeting_id: str | None = Query(
        None,
        description="If set, return only pending items for this meeting (single virtual page).",
    ),
) -> ReviewQueuePageOut:
    if meeting_id is not None and meeting_id.strip() != "":
        rows, n = await action_items_service.list_pending_review_for_meeting(meeting_id.strip())
        ps = max(1, len(rows))
        return ReviewQueuePageOut(
            items=[ActionItemReviewOut(**action_item_review_row(r)) for r in rows],
            page=1,
            page_size=ps,
            total_meetings=1 if n > 0 else 0,
            total_pending_items=n,
        )
